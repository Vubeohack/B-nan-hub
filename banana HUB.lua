...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>vietbloxfuit chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "vietbloxfuit", HidePremium = false, IntroText = "vietbloxfuit⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("vietbloxfuit SCRIPT")
W:AddLabel("Discordvietbloxfuit")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "vietbloxfuit(Noitification)",
